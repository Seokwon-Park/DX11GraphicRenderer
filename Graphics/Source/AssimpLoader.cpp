...
namespace graphics {
    void AssimpLoader::Load(std::string filename) {
        ...
        XMMATRIX tr = XMMatrixIdentity(); // Initial transformation
    ...
    void AssimpLoader::ProcessNode(aiNode* node, const aiScene* scene, XMMATRIX tr) {
    ...
    MeshData AssimpLoader::ProcessMesh(aiMesh* mesh, const aiScene* scene) {
        ...
        if (mesh->mMaterialIndex >= 0) {
            ...
            if (material->GetTextureCount(aiTextureType_DIFFUSE) > 0) {
                ...
                newMesh.textureFilename = "c:/zelda/"+std::string(std::filesystem::path(filepath.C_Str())
                    .filename()
                    .string());
            }
        }
